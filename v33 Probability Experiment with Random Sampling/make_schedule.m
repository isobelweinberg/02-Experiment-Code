 function [schedule, vars] = make_schedule(NumTrials, vars, option)
%% DISPLAY
%Control Trials
%allocate 1s to control trials, zeros to everything else
vars.NoControlTrials = NumTrials*vars.ControlProbability/100;
if round(vars.NoControlTrials) ~= vars.NoControlTrials
    error('vars.NoControlTrials is not a whole number!');
end
...
if round(vars.NoCoherentTrials) ~= vars.NoCoherentTrials
    error('vars.NoCoherentTrials is not a whole number!');
end
vars.NoTrialsPerCoherence = vars.NoCoherentTrials/numel(vars.CoherenceArray);
if round(vars.NoTrialsPerCoherence) ~= vars.NoTrialsPerCoherence
    error('vars.NoTrialsPerCoherence is not a whole number!');
end
...
schedule.coherence(vars.NoControlTrials+1:end) = repmat(vars.CoherenceArray, 1, vars.NoTrialsPerCoherence);
...
if option.TMS == 1
    ...
    if round(vars.NoFixedTrials) ~= vars.NoFixedTrials
        error('vars.NoFixedTrials is not a whole number!');
    end
