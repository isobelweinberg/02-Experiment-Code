try
    ...
    vars.Repeats = 132; %How many repeats of each condition %NB this does not include control trials %needs to be divisible by block number to give whole number
    ...
    vars.TMS.TimepointRangeEnd = 0.4;
    vars.TMS.FixedTimepointProb = 50;
    vars.TMS.RangeTimepointProb = 50;
    vars.TMS.TimepointRelations = 2; %usually 2 %range is currently hardcoded to be always relevant to 2
    ...
    if option.scaletoRT == 10
        ...
        if gridsize(1, 2) ~= numel(vars.LeftProbabilityArray)
            error('Wrong number of probabilities for RT scaling!')
        end
    end
    if vars.TMS.FixedTimepointProb + vars.TMS.RangeTimepointProb ~= 100
        error('Timepoint probabilitites do not add up to 100')
    end
    if option.scaletooverallRT == 1 &&  option.scaletoRT == 1
        error('Which kind of RT scaling do you want?')
    end
...
%         end
        ...
        vars.LeftProbabilityArray = vars.LeftProbabilityArray(randperm(length(vars.LeftProbabilityArray))); %fully random order of blocks
        ...
        for BlockNo=1:TotalNumBlocks
            ...
            data.main(:, 2) = BlockNo;
            data.main(:, 3) = vars.LeftProbabilityArray(1, BlockNo);
            ...
            [FixationXY, DotsXY, data, vars, schedule] = generate_stimuli(TrialsPerBlock, vars, params, option, data, participant); % Generate stimuli
