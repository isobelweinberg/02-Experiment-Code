...
vars.schedule = make_schedule(NumTrials, vars, option);
...
for TrialNo = 1:NumTrials
    ...
    %(4) - COHERENCE
    data.main(TrialNo, 4) = vars.schedule.coherence(TrialNo); 
    %(8) - NUM COHERENT DOTS
    ...
    data.main(TrialNo, 26) = vars.schedule.controloption(TrialNo); 
    %(6) - TIMEPOINT
    %(7) - TMS RELATION
    if option.TMS == 1
...
%             end 
%             if data.main(TrialNo,9) == 0
%                 data.main(TrialNo,9) = 1;
%                 errorcheck(1,TrialNo) = 1;
%             else
%                 errorcheck(1,TrialNo) = 0;
%             end
%         end
%     else
...
%     end
%     errorcheck
%     strcmp(vars.TMS.TimepointRelations{1, (data.main(TrialNo, 9))}, 'Fixation')
    %set TMS timepoint for the trial
%     if option.TMS == 1
        
        
%         if vars.schedule.TMSoption(TrialNo) == 1 && strcmp(vars.TMS.TimepointRelations{1, (data.main(TrialNo, 9))}, 'Fixation')
%             data.main(TrialNo, 7) = 1; %1 in the timepoint relation col means relative to Fixation
%         elseif vars.schedule.TMSoption(TrialNo) == 1 && strcmp(vars.TMS.TimepointRelations(1, (data.main(TrialNo, 9))), 'Stim')
%             data.main(TrialNo, 7) = 2; %2 in the timepoint relation col means relative to Stim
%         elseif vars.schedule.TMSoption(TrialNo) == 1 && strcmp(vars.TMS.TimepointRelations(1, (data.main(TrialNo, 9))), 'ITI')
