try
    ...
    vars.Repeats = 32; %How many repeats of each condition %NB this does not include control trials %needs to be divisible by block number to give whole number
    ...
    vars.ControlProbability = 50; %give as %; used to calculate vars.NoControlTrials later on - can specify this directly
    ...
    switch option.TMS
        case 1
            TotalNumTrials = vars.Repeats*numel(vars.CoherenceArray)*numel(vars.LeftProbabilityArray)...
                *(numel(vars.TMS.Timepoints)/(vars.TMS.Probability/100))/(vars.ControlProbability/100);
        case 0
            TotalNumTrials = vars.Repeats*numel(vars.CoherenceArray)*numel(vars.LeftProbabilityArray)/(vars.ControlProbability/100);
    end
